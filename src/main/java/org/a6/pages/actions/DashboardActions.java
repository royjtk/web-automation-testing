...
public class DashboardActions {
    ...
    public boolean isUserNameDisplayed(String username) {
        try {
            ...
            // Try alternative approach - look for text containing username anywhere in the header
            try {
        ...
        try {
            ...
            try {
                WebElement alternativeLogoutButton = driver.findElement(
                    By.xpath("//button[contains(@class, 'destructive') or contains(@class, 'logout') or .//svg[contains(@class, 'log-out')] or contains(text(), 'Logout') or contains(text(), 'Log Out') or contains(text(), 'Keluar')]"));
                ...
                try {
                    ...
                    WebElement anyLogoutElement = driver.findElement(
                        By.xpath("//*[contains(text(), 'Logout') or contains(text(), 'Log Out') or contains(text(), 'Keluar') or contains(@title, 'Logout')]"));
        ...
        try {
            ...
            try {
                ...
                WebElement alternativeHeader = driver.findElement(
                    By.xpath("//h1[contains(@class, 'font-secular') or contains(text(), 'Dasbor') or contains(text(), 'Dashboard')] | //div[contains(@class, 'dashboard-title')]"));
                return alternativeHeader.getText();
            } catch (Exception ex) {
                try {
                    // Try an even more generic approach
                    WebElement anyHeader = driver.findElement(
                        By.xpath("//*[contains(text(), 'Dasbor') or contains(text(), 'Dashboard') or contains(text(), 'Bendahara')]"));
